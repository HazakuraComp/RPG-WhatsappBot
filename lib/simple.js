const { default: makeWASocket, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, jidDecode, proto, getContentType, makeInMemoryStore, downloadContentFromMessage, fetchLatestWaWebVersion } = require("@ardsvt/baileys");
const { toAudio, toPTT, toVideo } = require('./converter')
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./exif')
const { smsg, color, getBuffer } = require("./myfunc")
...
if (new Date - (m?.messageTimestamp * 1000) > 60000) return;
if (global.mess.includes(m?.key.id)) return;
require("../client")(conn, m, chatUpdate, store)
global.mess.push(m?.key?.id)
 } catch (err) {
 console.log(err)
 }
});
